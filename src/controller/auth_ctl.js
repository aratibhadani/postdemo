...
module.exports = {
  ...
  // when user deleted also deleted his/her all post and articals
  // eslint-disable-next-line consistent-return
  deleteUser: async (req, res) => {
    ...
    try {
      ...
      // search post id where user id this
      const postIdArr = await postSchema.findAll({
        where: { userId },
        attributes: ['id'],
      }, { transaction: t });
      // search artical id where user id this
      const articalIdArr = await articalSchema.findAll({
        where: { userId },
        attributes: ['id'],
      }, { transaction: t });
      const postId = await returnArray(postIdArr);
      const articalId = await returnArray(articalIdArr);

      if (postId.length !== 0) {
        await postImageSchema.destroy({
          where: {
            postId: { [Op.in]: postId },
          },
        }, { transaction: t });
        await postSchema.destroy({ where: { userId } }, { transaction: t });
      }
      if (articalId.length !== 0) {
        await articalImageSchema.destroy({
          where: {
            postId: { [Op.in]: articalId },
          },
        }, { transaction: t });
        await articalSchema.destroy({ where: { userId } }, { transaction: t });
      }
